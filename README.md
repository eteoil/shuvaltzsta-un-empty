...
画面下のボタン（クリック・タップ）かキーボード。PC ではボタンの横に対応するキーが表示される。矢印キーで動かすなら左手で X・Z、WASD で動かすなら右手で K・J が A・B になる。タイトルでは START で PRESS START からメニューへ進み、NEW GAME を選ぶ。左上のランプはボタンを押している間だけ光る。
