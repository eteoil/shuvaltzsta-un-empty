...
画面下のボタン（クリック・タップ）かキーボード。PC ではボタンの横に対応するキーが表示される。タイトルでは START で PRESS START からメニューへ進み、NEW GAME を選ぶ。左上のランプはボタンを押している間だけ光る。

| ボタン | キーボード | 探索 | 戦闘 |
| --- | --- | --- | --- |
| 十字 | 矢印キー | 移動（上＝北東・右＝南東・下＝南西・左＝北西） | 移動（同じ） |
| A | A / X / Space | 話す（向き合って押す）・会話を送る | 隣の敵を攻撃 |
| B | B / Z | 会話を送る | 回避 |
| PAUSE | P / Esc | ポーズ | ポーズ |
...
- **紫のマス**は B では避けられない大技。歩いて範囲の外へ出るしかない。敵は隣に張り付かれたときだけ、この大技を選ぶ（続けては出さない）。
- 敵には向きがある。**背中や横から殴るとダメージ2倍**で、`GUARD` 中でも通る。敵は行動するたびにこちらへ向き直るので、そのすきに回り込む。
- 枠だけの赤いマスはフェイント。B を押すと空振りになり、コンボが切れる。
- 敵の頭上に `CHANCE` が出ている間は、その敵へのダメージが2倍。`GUARD` の間は正面からだと弾かれる。
- 1拍につき1アクション（A か B）。
